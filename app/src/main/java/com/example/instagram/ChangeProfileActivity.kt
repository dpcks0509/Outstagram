    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.change_img).setOnClickListener {
            ...
            val requestFile = RequestBody.create(
                MediaType.parse(this.contentResolver.getType(imageUri!!)),file)
            ...
            retrofitService.changeProfile(userId, header, body, user).enqueue(object: Callback<Any> {
                override fun onResponse(call: Call<Any>, response: Response<Any>) {
                    if(response.isSuccessful) {
                        Toast.makeText(this@ChangeProfileActivity, "프로필 변경 완료", Toast.LENGTH_SHORT).show()
                        onBackPressed()
                    }
                }
                override fun onFailure(call: Call<Any>, t: Throwable) {
                    Toast.makeText(this@ChangeProfileActivity, "프로필 변경 실패", Toast.LENGTH_SHORT).show()
                }
            })
        }
    }
